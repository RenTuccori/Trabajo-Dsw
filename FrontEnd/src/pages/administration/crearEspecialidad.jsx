...
import { toast } from 'react-toastify'; // Importé la librería de toastify
...
export function CrearEspecialidad() {
  ...
  const { especialidades, crearEspecialidad, ObtenerEspecialidadesDisponibles, borrarEspecialidad } = useAdministracion();
  ...
  return (
    <div className="min-h-screen bg-gradient-to-b from-blue-100 to-white flex items-center justify-center p-4">
      <div className="bg-white rounded-lg shadow-md w-full max-w-md p-6 space-y-4">
        <h2 className="text-xl font-semibold text-center text-gray-800">Crear Nueva Especialidad</h2>
        ...
        <h3 className="text-lg font-medium text-gray-800 mt-6">Especialidades Creadas</h3>
        <ul className="space-y-2">
          {especialidades.length > 0 ? (
            especialidades.map((especialidad) => (
              <li key={especialidad.idEspecialidad} className="bg-gray-100 p-4 rounded-lg flex justify-between items-center">
                <span>
                  <strong>{especialidad.nombre}</strong>
                </span>
                <button
                  onClick={() => handleBorrarEspecialidad(especialidad.idEspecialidad)}
            ...
            <p className="text-center text-gray-600">No hay especialidades creadas aún.</p>
          )}
        </ul>
      </div>
    </div>
