...
describe('Validation Middleware Tests', () => {
  ...
  describe('validateUserLogin', () => {
    it('should pass with valid data', async () => {
      const req = { body: { nationalId: 12345678, password: 'secret123' } };
      const res = await runValidation(validateUserLogin, req);
      expect(res._status).toBeNull();
    });

    it('should fail with invalid DNI', async () => {
      const req = { body: { nationalId: 123, password: 'secret123' } };
      const res = await runValidation(validateUserLogin, req);
    ...
    it('should fail when password is missing', async () => {
      const req = { body: { nationalId: 12345678 } };
      const res = await runValidation(validateUserLogin, req);
      expect(res._status).toBe(400);
    });
  });
  ...
  describe('validateCreateUser', () => {
    it('should pass with all required fields', async () => {
      const req = { body: { nationalId: 33333333, password: 'pass123', birthDate: '2000-06-15', firstName: 'Ana', lastName: 'Lopez' } };
      const res = await runValidation(validateCreateUser, req);
      expect(res._status).toBeNull();
    });

    it('should fail with empty firstName', async () => {
      const req = { body: { nationalId: 33333333, password: 'pass123', birthDate: '2000-06-15', firstName: '', lastName: 'Lopez' } };
      const res = await runValidation(validateCreateUser, req);
      expect(res._status).toBe(400);
    });
  });
  ...
  describe('validateAppointmentAction', () => {
    it('should pass with valid appointment id', async () => {
      const req = { body: { id: 5 } };
