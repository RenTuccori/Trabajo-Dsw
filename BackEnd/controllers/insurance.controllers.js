...
export const getInsurances = async (req, res) => {
    try {
        console.log('🏥 BACKEND - getInsurances: Obteniendo obras sociales');
        const [result] = await pool.query('SELECT * FROM insurance_companies WHERE status = \'Habilitado\'');
        console.log('🗄️ BACKEND - Obras sociales encontradas:', result);
        console.log('📊 BACKEND - Número de obras sociales:', result.length);
        
        if (result.length === 0) {
            console.log('⚠️ BACKEND - No hay obras sociales habilitadas');
            return res.status(404).json({ message: 'No enabled insurance companies' });
        } else {
            console.log('✅ BACKEND - Enviando obras sociales');
            res.json(result);
        }
    } catch (error) {
        console.log('💥 BACKEND - Error en getInsurances:', error);
...
export const getInsuranceById = async (req, res) => {
    try {
        ...
        if (result.length === 0) {
            return res.status(404).json({ message: 'Insurance company not found or not enabled' });
        } else {
            res.json(result[0]);
        }
    } catch (error) {
